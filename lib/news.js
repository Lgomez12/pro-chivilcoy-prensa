...
function isLaRazonUtilityPage(url = '', title = '') {
  ...
  return blockedPathBits.some((x) => u.includes(x)) || blockedTitles.some((x) => t === x || t.startsWith(x));
}

function normalizeLaRazonUrl(raw) {
  try {
    const cleaned = decodeEntities(String(raw || '').trim());
    const u = new URL(cleaned, 'https://www.larazondechivilcoy.com.ar');
...
function parseLaRazonHtml(html = '', baseUrl = 'https://www.larazondechivilcoy.com.ar') {
  const out = [];
  const seen = new Set();
  const re = /<a\s+[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;
  let m;
  while ((m = re.exec(html)) !== null) {
    ...
    if (isLaRazonUtilityPage(url, title)) continue;
    const around = cleanExcerpt(html.slice(Math.max(0, m.index - 900), Math.min(html.length, m.index + m[0].length + 1400)));
    if (!isRelevant(`${title} ${around}`) && !isLocalPoliticalArticle(`${title} ${around}`, 'La Razón de Chivilcoy')) continue;

    seen.add(key);
    out.push({
      date: parseSpanishDate(around),
      title,
      source: 'La Razón de Chivilcoy',
      url,
      people: inferPeople(`${title} ${around}`),
      topics: inferTopics(`${title} ${around}`),
...
async function fetchLaRazon() {
  ...
  for (const search of laRazonSearches) {
    ...
    while (queue.length && seenPages.size < search.maxPages) {
      ...
      // 1) Intento directo: si el diario deja pasar la solicitud, parseamos HTML.
      const direct = await safeFetch(pageUrl);
      if (direct) all.push(...parseLaRazonHtml(direct, pageUrl));

      // 2) Fallback por Jina Reader: devuelve markdown aun cuando el origen bloquee Vercel.
      ...
      for (const p of parseLaRazonPagination(md, search.url)) {
        if (!seenPages.has(p) && queue.length < 8) queue.push(p);
      }
    }
  }

  // Las tarjetas del buscador de La Razón suelen venir con fecha en el contexto,
  // pero si alguna no la trae, completamos metadata desde la nota individual.
  const enriched = await Promise.all(dedupe(all).map(enrichArticle));
