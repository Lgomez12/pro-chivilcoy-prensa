...
async function fetchLaRazonIndexed() {
  // Dos páginas por búsqueda. Se hacen en paralelo para no penalizar demasiado el tiempo de respuesta.
  const urls = [];
  for (const query of laRazonIndexedQueries) {
    urls.push(bingRssUrl(query, 1));
    urls.push(bingRssUrl(query, 11));
  }
