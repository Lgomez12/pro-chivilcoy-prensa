...
function cleanTitle(str = '') {
  ...
  const junk =
    /\s+(?:decoding|loading|srcset|sizes|src|class|style|width|height|fetchpriority|aria-[\w-]+)=/i;
...
function inferPeople(text = '') {
  ...
  if (t.includes('libertad avanza')) {
    p.push('La Libertad Avanza');
  }
...
function inferTopics(text = '') {
  ...
  const map = [
    ...
    ['obra', 'Obras públicas'],
    ['tasa', 'Tasas'],
    ['rafam', 'Rendición de cuentas'],
    ['cuentas', 'Rendición de cuentas'],
    ['empleado', 'Empleo municipal'],
    ['municipal', 'Gestión municipal'],
    ['transpar', 'Transparencia'],
    ['concejo', 'HCD'],
    ['hcd', 'HCD'],
    ['terminal', 'Gestión municipal'],
    ['comercio', 'Comercio']
  ];
...
function parseFeed(xml, sourceName) {
  ...
  return blocks
    .map((block) => {
      ...
      const description =
        cleanExcerpt(rawDescription);

      const pub =
        stripTags(
          getTagRaw(block, 'pubDate') ||
          getTagRaw(block, 'published') ||
          getTagRaw(block, 'updated')
        );
...
async function enrichArticle(article) {
  const suspiciousTitle =
    /(?:decoding=|loading=|srcset=|sizes=|https?:\/\/|<img|\/>)|.{220,}/i.test(
      article.title || ''
    );
  ...
  return {
    ...
    date:
      ...
      article.date,
    people:
      inferPeople(combined),
    topics:
      inferTopics(combined)
  };
}

function isLaRazonUtilityPage(
  url = '',
  title = ''
) {
  let path = '';
...
function pickLaRazonTitleFromContext(
  fragment = '',
  anchorText = ''
) {
  const candidates = [];
...
function parseLaRazonPagination(
  md = '',
  searchUrl
) {
  const urls = [];
...
function extractLaRazonUrlFromBing(
  ...
  for (
    ...
    const embedded =
      decoded.match(
        /https?:\/\/(?:www\.)?larazondechivilcoy\.com\.ar\/noticias\/[^\s"'<>]+/i
      )?.[0];
...
async function fetchLaRazonIndexed() {
  ...
  for (
    ...
    if (md) {
      directResults.push(
        ...parseLaRazonMarkdown(
          md
        )
      );

      const pagination =
        parseLaRazonPagination(
          md,
          searchUrl
        );
...
function validArticle(a) {
  ...
  const text =
    `${a.title} ${a.excerpt || ''} ${(a.people || []).join(' ')}`;
