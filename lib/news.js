...
function extractLaRazonUrlFromBing(rawLink = '', description = '') {
  const candidates = [rawLink, ...extractHrefValues(description)];

  for (const raw of candidates) {
    if (!raw) continue;
    const decoded = decodeUrlRepeatedly(raw);

    // A veces Bing entrega directamente la URL del medio.
    const direct = normalizeLaRazonUrl(decoded);
    if (direct) return direct;

    // Bing suele envolver la URL original en ?url= o ?u=.
    try {
      const u = new URL(decoded, 'https://www.bing.com');
      const params = ['url', 'u', 'r', 'target'];
      for (const key of params) {
        const v = u.searchParams.get(key);
        if (!v) continue;
        const original = normalizeLaRazonUrl(decodeUrlRepeatedly(v));
        if (original) return original;
      }
    } catch {}

    // Último respaldo: buscar una URL de La Razón embebida/codificada dentro del string.
    const embedded = decoded.match(/https?:\/\/(?:www\.)?larazondechivilcoy\.com\.ar\/noticias\/[^\s"'<>]+/i)?.[0];
    if (embedded) {
      const original = normalizeLaRazonUrl(embedded);
      if (original) return original;
    }
  }
  return null;
...
function parseBingRssForLaRazon(xml = '') {
  const blocks = xml.match(/<item\b[\s\S]*?<\/item>/gi) || [];
  const out = [];

  for (const block of blocks) {
    const title = cleanTitle(getTagRaw(block, 'title'));
    const rawDescription = getTagRaw(block, 'description');
    const excerpt = cleanExcerpt(rawDescription);
    const date = normalizeDateValue(stripTags(getTagRaw(block, 'pubDate')));
    const bingLink = getLink(block);
    const url = extractLaRazonUrlFromBing(bingLink, rawDescription);
    if (!url || !title || !date) continue;
    if (isLaRazonUtilityPage(url, title)) continue;

    const combined = `${title} ${excerpt}`;
    if (!isLocalPoliticalArticle(combined, 'La Razón de Chivilcoy') && !isRelevant(combined)) continue;
...
function bingRssUrl(query, first = 1) {
  const q = encodeURIComponent(query);
  return `https://www.bing.com/search?q=${q}&format=rss&setlang=es-AR&cc=AR&first=${first}`;
}
...
async function fetchLaRazonIndexed() {
  const directTerms = [
    ...
    'LLA PRO',
    'PRO Chivilcoy',
    'La Libertad Avanza Chivilcoy',
    'La Libertad Avanza PRO'
  ];

  const directResults = [];

  // 1. Buscar directamente dentro de La Razón
  for (const term of directTerms) {
    const searchUrl =
      `https://larazondechivilcoy.com.ar/noticias/buscar/?buscar=${encodeURIComponent(term)}`;

    // Primero mediante Jina, que suele poder leer mejor sitios que bloquean bots
    const md = await safeFetch(searchUrl, { jina: true });

    if (md) {
      directResults.push(...parseLaRazonMarkdown(md));

      // Revisar también las páginas siguientes del buscador
      const pagination = parseLaRazonPagination(md, searchUrl);

      for (const pageUrl of pagination.slice(0, 4)) {
        const pageMd = await safeFetch(pageUrl, { jina: true });
        if (pageMd) {
          directResults.push(...parseLaRazonMarkdown(pageMd));
        }
      }
    }

    // Segundo intento: HTML directo del sitio
    ...
    if (html) {
      directResults.push(
        ...parseLaRazonHtml(
          html,
          'https://larazondechivilcoy.com.ar'
        )
      );
    }
  }

  // 2. Mantener Bing como respaldo
  const bingUrls = [];

  for (const query of laRazonIndexedQueries) {
    bingUrls.push(bingRssUrl(query, 1));
    bingUrls.push(bingRssUrl(query, 11));
  }

  const bingResponses = await Promise.all(
    bingUrls.map(async (url) => {
      const xml = await safeFetch(url);
      return xml ? parseBingRssForLaRazon(xml) : [];
    })
  );

  const bingResults = bingResponses.flat();

  // 3. Unir búsqueda directa + Bing
  return dedupe([...directResults, ...bingResults])
    .filter((a) => a.title && a.url)
    .filter((a) => !isLaRazonUtilityPage(a.url, a.title));
}
  ...
  return dedupe(responses.flat())
    .filter((a) => a.title && a.url && a.date && isWithinRange(a.date))
    .filter((a) => !isLaRazonUtilityPage(a.url, a.title));
}
...
export async function getAllNews() {
  ...
  const laRazon = await fetchLaRazonIndexed();
